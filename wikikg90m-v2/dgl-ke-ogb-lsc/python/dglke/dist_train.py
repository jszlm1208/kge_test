...
class ArgParser(CommonArgParser):
    def __init__(self):
        ...
        self.add_argument(
            '--path', type=str, help='path of distributed workspace.')
...
--regularization_coef %f --num_client %d''' % (
        args.model_name, args.dataset, args.data_path, args.format,
        args.ip_config, args.hidden_dim, args.gamma, args.lr, total_client,
        args.model_name, args.dataset, args.data_path, args.format,
        args.save_path, args.ip_config, args.batch_size, args.neg_sample_size,
        args.hidden_dim, args.gamma, args.lr, args.max_step, args.log_interval,
        args.num_thread, args.batch_size_eval, args.neg_sample_size_eval,
        args.regularization_coef, args.num_client_proc)
    if args.test == True:
        cmd_str += ' --test'
...
def launch(args):
    ...
    with open(args.ip_config) as f:
        ...
        for line in f:
            ...
            cmd_str = 'cd %s; ./%s %d %d' % (args.path, SCRIPT_FILE, id_low,
                                             id_high)
