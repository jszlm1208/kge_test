...
def get_server_data(args, machine_id):
    """Get data from data_path/dataset/part_machine_id
      ...
      Return: glocal2local, 
              ...
              relation_emb_state
   """
    g2l, dataset = get_server_partition_dataset(args.data_path, args.dataset,
                                                machine_id)
...
def start_server(args):
    ...
    if my_server.get_id() % my_server.get_group_count() == 0:  # master server
        g2l, entity_emb, entity_emb_state, relation_emb, relation_emb_state = get_server_data(
            args, my_server.get_machine_id())
        ...
        my_server.init_data(
            name='relation_emb_state', data_tensor=relation_emb_state)
        my_server.init_data(name='entity_emb', data_tensor=entity_emb)
        my_server.init_data(
            name='entity_emb_state', data_tensor=entity_emb_state)
    else:  # backup server
