...
def batched_l2_dist(a, b):
    ...
    squared_res = th.baddbmm(
        b_squared.unsqueeze(-2), a, b.transpose(-2, -1),
...
class TransEScore(nn.Module):
    ...
    def infer(self, head_emb, rel_emb, tail_emb):
        ...
        score = (head_emb + rel_emb
                 ).unsqueeze(2) - tail_emb.unsqueeze(0).unsqueeze(0)
...
class TransRScore(nn.Module):
    ...
    def prepare(self, g, gpu_id, trace=False):
        ...
        g.edata['head_emb'] = th.einsum('ab,abc->ac', g.ndata['emb'][head_ids],
                                        projection)
        g.edata['tail_emb'] = th.einsum('ab,abc->ac', g.ndata['emb'][tail_ids],
                                        projection)
    ...
    def prepare_cross_rels(self, cross_rels):
        self.projection_emb.setup_cross_rels(cross_rels,
                                             self.global_projection_emb)

    def writeback_local_emb(self, idx):
        self.global_projection_emb.emb[idx] = self.projection_emb.emb.cpu()[
            idx]
...
class DistMultScore(nn.Module):
    ...
    def infer(self, head_emb, rel_emb, tail_emb):
        ...
        rel_emb = rel_emb.unsqueeze(0)
        score = (head_emb *
                 rel_emb).unsqueeze(2) * tail_emb.unsqueeze(0).unsqueeze(0)
    ...
    def create_neg(self, neg_head):
        if neg_head:
            ...
            def fn(heads, relations, tails, num_chunks, chunk_size,
                ...
                tmp = (tails * relations).reshape(num_chunks, chunk_size,
                                                  hidden_dim)
...
class RESCALScore(nn.Module):
    ...
    def infer(self, head_emb, rel_emb, tail_emb):
        ...
        score = head_emb * th.einsum('abc,dc->adb', rel_emb,
                                     tail_emb).unsqueeze(0)
    ...
    def create_neg(self, neg_head):
        if neg_head:
            ...
            def fn(heads, relations, tails, num_chunks, chunk_size,
                ...
                relations = relations.view(-1, self.relation_dim,
                                           self.entity_dim)
...
class RotatEScore(nn.Module):
    ...
    def infer(self, head_emb, rel_emb, tail_emb):
        ...
        re_rel, im_rel = th.cos(phase_rel), th.sin(phase_rel)
        re_score = re_head.unsqueeze(1) * re_rel.unsqueeze(
            0) - im_head.unsqueeze(1) * im_rel.unsqueeze(0)
        im_score = re_head.unsqueeze(1) * im_rel.unsqueeze(
            0) + im_head.unsqueeze(1) * re_rel.unsqueeze(0)
    ...
    def create_neg(self, neg_head):
        ...
        if neg_head:
            ...
            def fn(heads, relations, tails, num_chunks, chunk_size,
                ...
                imag = -emb_real * rel_imag + emb_imag * rel_real
                emb_complex = th.cat((real, imag), dim=-1)
                tmp = emb_complex.reshape(num_chunks, chunk_size, 1,
                                          hidden_dim)
                heads = heads.reshape(num_chunks, 1, neg_sample_size,
                                      hidden_dim)
                score = tmp - heads
                score = th.stack(
                    [
...
class SimplEScore(nn.Module):
    ...
    def edge_func(self, edges):
        ...
        score = th.clamp(1 / 2 * (forward_score + backward_score).sum(-1), -20,
                         20)
        return {'score': score}

    def infer(self, head_emb, rel_emb, tail_emb):
        head_i, head_j = th.chunk(head_emb.unsqueeze(1), 2, dim=-1)
        tail_i, tail_j = th.chunk(
            tail_emb.unsqueeze(0).unsqueeze(0), 2, dim=-1)
    ...
    def create_neg(self, neg_head):
        if neg_head:
            ...
            def fn(heads, relations, tails, num_chunks, chunk_size,
                ...
                forward_tmp = (rel * tail_j).reshape(num_chunks, chunk_size,
                                                     hidden_dim // 2)
                backward_tmp = (rel_inv * tail_i).reshape(
                    num_chunks, chunk_size, hidden_dim // 2)
                heads = heads.reshape(num_chunks, neg_sample_size, hidden_dim)
                heads = th.transpose(heads, 1, 2)
                head_i = heads[..., :hidden_dim // 2, :]
                head_j = heads[..., hidden_dim // 2:, :]
                tmp = 1 / 2 * (
                    th.bmm(forward_tmp, head_i) + th.bmm(backward_tmp, head_j))
            ...
            def fn(heads, relations, tails, num_chunks, chunk_size,
                ...
                rel = relations[..., :hidden_dim // 2]
                rel_inv = relations[..., hidden_dim // 2:]
                forward_tmp = (head_i * rel).reshape(num_chunks, chunk_size,
                                                     hidden_dim // 2)
                backward_tmp = (rel_inv * head_j).reshape(
                    num_chunks, chunk_size, hidden_dim // 2)
                tails = tails.reshape(num_chunks, neg_sample_size, hidden_dim)
                tails = th.transpose(tails, 1, 2)
                tail_i = tails[..., :hidden_dim // 2, :]
                tail_j = tails[..., hidden_dim // 2:, :]
                tmp = 1 / 2 * (
                    th.bmm(forward_tmp, tail_j) + th.bmm(backward_tmp, tail_i))
                score = th.clamp(tmp, -20, 20)
                return score
