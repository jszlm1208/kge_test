...
class InferModel(object):
    ...
    def load_emb(self, path, dataset):
        ...
        self.relation_emb.load(path,
                               dataset + '_' + self.model_name + '_relation')
...
class MLP(torch.nn.Module):
    def __init__(self, input_entity_dim, entity_dim, input_relation_dim,
        ...
        self.transform_r_net = torch.nn.Linear(input_relation_dim,
                                               relation_dim)
...
class KEModel(object):
    ...
    def __init__(self,
        ...
        if not self.strict_rel_part and not self.soft_rel_part:
            ...
            self.global_relation_emb = ExternalEmbedding(args, n_relations,
                                                         rel_dim, F.cpu())
        ...
        if model_name == 'TransE' or model_name == 'TransE_l2':
            ...
            projection_emb = ExternalEmbedding(args, n_relations,
                                               ...
                                               F.cpu()
                                               if args.mix_cpu_gpu else device)

            self.score_func = TransRScore(gamma, projection_emb, relation_dim,
                                          entity_dim)
        elif model_name == 'DistMult':
            self.score_func = DistMultScore()
        elif model_name == 'ComplEx':
    ...
    def prepare_relation(self, device=None):
        ...
        self.relation_emb = ExternalEmbedding(self.args, self.n_relations,
                                              self.rel_dim, device)
        self.relation_emb.init(self.emb_init)
        if self.model_name == 'TransR':
            local_projection_emb = ExternalEmbedding(
                self.args, self.n_relations, self.entity_dim * self.rel_dim,
                device)
    ...
    def pull_model(self, client, pos_g, neg_g):
        with th.no_grad():
            entity_id = F.cat(seq=[pos_g.ndata['id'], neg_g.ndata['id']],
                              dim=0)
            ...
            entity_data = client.pull(
                name='entity_emb', id_tensor=global_entity_id)
            relation_data = client.pull(
                name='relation_emb', id_tensor=relation_id)
