...
def load_model_from_checkpoint(args, n_entities, n_relations, ckpt_path,
                               ent_feat_dim, rel_feat_dim):
    model = load_model(args, n_entities, n_relations, ent_feat_dim,
                       rel_feat_dim)
...
def train(args,
    ...
    for step in range(0, args.max_step):
        ...
        if (step + 1) % args.log_interval == 0:
            if (client is not None) and (client.get_machine_id() != 0):
                ...
                for k in logs[0].keys():
                    ...
                    logging.info('[proc {}][Train]({}/{}) average {}: {}'.
                                 format(rank, (step + 1), args.max_step, k, v))
                logs = []
                logging.info(
                    '[proc {}][Train] {} steps take {:.3f} seconds'.format(
                        rank, args.log_interval, time.time() - start))
                logging.info(
                    '[proc {}]sample: {:.3f}, forward: {:.3f}, backward: {:.3f}, update: {:.3f}'.
                    format(rank, sample_time, forward_time, backward_time,
                           update_time))
                sample_time = 0
                update_time = 0
                forward_time = 0
        ...
        if args.valid and (
                step + 1
        ) % args.eval_interval == 0 and valid_samplers is not None:
            ...
            if barrier is not None:
                barrier.wait()
            logging.info(
                '[proc {}]barrier wait in validation take {:.3f} seconds:'.
                format(rank, time.time() - valid_start))
            valid_start = time.time()
            if valid_samplers is not None:
                valid_input_dict = test(
                    args, model, valid_samplers, step, rank, mode='Valid')
                th.save(valid_input_dict,
                        os.path.join(args.save_path,
                                     "valid_{}_{}.pkl".format(rank, step)))
            if test_samplers is not None:
                test_input_dict = test(
                    args, model, test_samplers, step, rank, mode='Test')
                th.save(test_input_dict,
                        os.path.join(args.save_path,
                                     "test_{}_{}.pkl".format(rank, step)))
            logging.info('[proc {}]validation and test take {:.3f} seconds:'.
                         format(rank, time.time() - valid_start))
            if args.soft_rel_part:
                model.prepare_cross_rels(cross_rels)
            if barrier is not None:
...
def train_mp(args,
    ...
    train(args, model, train_sampler, valid_samplers, test_samplers, rank,
          rel_parts, cross_rels, barrier)
